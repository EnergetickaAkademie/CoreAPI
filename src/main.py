...
import logging
import traceback
from state import GameState, available_scripts, BoardState, get_fresh_script
...
def poll_binary():
    ...
    try:
        ...
        if not script:
            # Return empty response when no game is active, with empty connected buildings
        ...
        # Pack the data including board's connected buildings for proper state synchronization
        response = BoardBinaryProtocol.pack_coefficients_response(
...
def start_game_scenario():
    ...
    # Obtain a fresh script instance each start to avoid leaking prior round index/state
    # (previous implementation reused a shared instance from available_scripts)
    script = get_fresh_script(scenario_id)

    # Before assigning the new script, clear per-board transient connection data so a new
    # scenario starts with a "clean sheet" (requirement: do not carry over connected buildings).
    # We intentionally only clear connected buildings & connection-derived arrays; we keep
    # historical production/consumption lists so lecturers can still inspect previous game
    # statistics unless a full reset endpoint is introduced later.
    for board in user_game_state.boards.values():
        board.clear_connected_buildings()
        board.replace_connected_consumption([])
        board.replace_connected_production([])
        board.power_generation_by_type = {}
...
def next_round():
    ...
    if script.step():
        ...
        # Natural end of game: finalize boards (saves last round, clears connected buildings)
        try:
            user_game_state.finalize_all_boards_current_round()
        except Exception as e:
            logger.error(f"Error finalizing boards at game end: {e}")
        return jsonify({
...
def end_game():
    ...
    # Get user's game state
    user_game_state = get_user_game_state(request.user)
    # Finalize & clear connected buildings to ensure no carry-over if a new scenario starts
    try:
        user_game_state.finalize_all_boards_current_round()
    except Exception as e:
        logger.error(f"Error finalizing boards on manual end: {e}")
    # Reset script to null/none after finalization
